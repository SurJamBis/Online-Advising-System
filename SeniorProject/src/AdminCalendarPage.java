...
public class AdminCalendarPage extends  JPanel{
    private final int month;
    private int year;
    static  ArrayList<String> dateList = new ArrayList<>();
    static  ArrayList<String> monthyearList = new ArrayList<>();
 ...
 */
    ...
    public AdminCalendarPage(int y ,int m,JLabel dateview,Container parrent) throws  IllegalArgumentException{
...
int    daysInMoth  =  this.DAYS_IN_MONTH[month];
  LineBorder  bb =new LineBorder(Color.LIGHT_GRAY);
        EmptyBorder eb =  new EmptyBorder(12, 12, 12, 12);
...
try
{
	...
	//ResultSet rs = st.executeQuery("select Date,Month_Year from onlinesystem.meeting");
	ResultSet rs = st.executeQuery("SELECT Date, Month_Year FROM meeting GROUP BY date HAVING COUNT(date) >1");
	while(rs.next())
	{
		dateList.add(rs.getString(1));
		monthyearList.add(rs.getString(2));
...
for(int i = 1; i <= daysInMoth; i++){
	 ...
	 //  Iterator itr = dateList.iterator()
	    l.addMouseListener(new MouseAdapter() {
	    	...
	    	{	  /*
	    		for(int z= 0; z<monthyearList.size(); z++)
	    		{
	    			if (monthyearList.get(z).equals(dateview.getText()))
	    			{
    					if (dateList.get(z).equals(l.getText()))
	    				//if(dateList.next().equals(l.getText()))
    					{
    						//JOptionPane.showMessageDialog(null,dateList.get(z));
    						SelectTime appp = new SelectTime();
    						appp.setVisible(true);
    						break;
    					}
    			}
    		}  		
	  	    		//JOptionPane.showMessageDialog(null,dateview);
	    	 	//	JOptionPane.showMessageDialog(null, page.add(l));
	    	}
	    	*/
	    	}
	    });
	        
    l.setBackground(rgb(200	,200,200));
       l.setFont(getFont());
       l.setBorder(new CompoundBorder(bb, eb));
       page.add(l);
